...
const tableName = "Gen_Provincia";
const idField = "Gen_id_provincia";
const fields = [
...
provinciaRouter.post("/", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: `Error al crear ${tableName}: ${error.message}` });
  }
});

provinciaRouter.put("/:id", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: `Error al actualizar ${tableName}: ${error.message}` });
  }
});

provinciaRouter.delete("/:id", async (req, res) => {
  try {
    await query(`DELETE FROM ${tableName} WHERE ${idField} = ?`, [
      req.params.id,
    ]);
