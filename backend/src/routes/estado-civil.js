...
const tableName = "Gen_EstadoCivil";
const idField = "Gen_id_estado_civil";
const fields = [
...
const getValuesFromRequestBody = (body) => {
  return fields.map((f) => {
    ...
    if (value === undefined || value === "") {
      // Retorna el default si está definido, si no, retorna null
...
estadoCivilRouter.get("/", async (req, res) => {
  try {
...
estadoCivilRouter.get("/:id", async (req, res) => {
  try {
...
estadoCivilRouter.post("/", async (req, res) => {
  try {
...
estadoCivilRouter.put("/:id", async (req, res) => {
  try {
    const setClause = fields.map((f) => `${f.name} = ?`).join(", ");
    const values = [
      ...getValuesFromRequestBody(req.body),
      req.params.id, // El ID va al final para la cláusula WHERE
    ];
...
estadoCivilRouter.delete("/:id", async (req, res) => {
  try {
